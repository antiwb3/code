...
// QtPropertyTextEdit
class QtPropertyTextEdit :public QTextEdit
{
    Q_OBJECT
Q_SIGNALS:
    void editFinished();
...
// QtStringMgrEditWidget
class QtStringMgrEditWidget  : public QWidget
{
    Q_OBJECT
public:
    QtStringMgrEditWidget(QWidget *parent);
...
private:
    QtPropertyTextEdit *m_label;
    QToolButton *m_button;
};

QtStringMgrEditWidget::QtStringMgrEditWidget(QWidget *parent) 
: QWidget(parent)
, m_label(NULL)
, m_button(NULL)
{
    m_label = new QtPropertyTextEdit;
    ...
    connect(m_label, SIGNAL(editFinished ()), this, SLOT(editingFinished()));

    lt->addWidget(m_button);
}

void QtStringMgrEditWidget::buttonClicked()
{
    int ok = false;
	QString newValue;
	QString dialogName = DialogMgr::GetDialogName();
    QString oldValue = m_label->text();
	ok = DialogMgr::getText(this, dialogName, oldValue, newValue, 0);
...
void QtStringMgrEditWidget::setValue(const QString &value)
{
    QString oldValue = m_label->text();
    if (oldValue != value) {
...
void QtStringMgrEditWidget::editingFinished()
{
    emit valueChanged(m_label->text());
}

bool QtStringMgrEditWidget::eventFilter(QObject *obj, QEvent *ev)
{
    if (obj == m_button) {
        switch (ev->type()) {
...
class QtStringMgrEditorFactoryPrivate : public EditorFactoryPrivate<QtStringMgrEditWidget>
{
    QtStringMgrEditorFactory *q_ptr;
    Q_DECLARE_PUBLIC(QtStringMgrEditorFactory)
public:
...
void QtStringMgrEditorFactoryPrivate::slotPropertyChanged(QtProperty *property,
                                                      const QString &value)
{
    const PropertyToEditorListMap::iterator it = m_createdEditors.find(property);
    if (it == m_createdEditors.end())
        return;
    QListIterator<QtStringMgrEditWidget *> itEditor(it.value());

    while (itEditor.hasNext())
        itEditor.next()->setValue(value);
}

void QtStringMgrEditorFactoryPrivate::slotSetValue(const QString &value)
{
    QObject *object = q_ptr->sender();
...
QtStringMgrEditorFactory::QtStringMgrEditorFactory(QObject *parent) :
QtAbstractEditorFactory<QtStringPropertyManager>(parent),
d_ptr(new QtStringMgrEditorFactoryPrivate())
{
    d_ptr->q_ptr = this;
}

QtStringMgrEditorFactory::~QtStringMgrEditorFactory()
{
    qDeleteAll(d_ptr->m_editorToProperty.keys());
    delete d_ptr;
}
...
/*!
    ...
    Reimplemented from the QtAbstractEditorFactory class.
*/
void QtStringMgrEditorFactory::connectPropertyManager(QtStringPropertyManager *manager)
{
...
/*!
    ...
    Reimplemented from the QtAbstractEditorFactory class.
*/
QWidget *QtStringMgrEditorFactory::createEditor(QtStringPropertyManager *manager,
        QtProperty *property, QWidget *parent)
{
    QtStringMgrEditWidget *editor = d_ptr->createEditor(property, parent);
    editor->setValue(manager->value(property));
...
/*!
    ...
    Reimplemented from the QtAbstractEditorFactory class.
*/
void QtStringMgrEditorFactory::disconnectPropertyManager(QtStringPropertyManager *manager)
{
...
void QtPropertyTextEdit::focusOutEvent(QFocusEvent * event)
{
	editFinished();
}

// QtInputEditWidget
class QtInputEditWidget  : public QWidget
{
    Q_OBJECT
public:
    QtInputEditWidget(QWidget *parent);
    bool eventFilter(QObject *obj, QEvent *ev);
...
QtInputEditWidget::QtInputEditWidget(QWidget *parent) 
: QWidget(parent)
, m_label(NULL)
, m_button(NULL)
{
...
void QtInputEditWidget::buttonClicked()
{
	QString newValue;
	QString oldValue = m_label->text();
	QString dialogName = DialogMgr::GetDialogName();
	int ok = DialogMgr::getText(this, dialogName, oldValue, newValue, 0);
...
void QtInputEditWidget::EditingFinished()
{
	emit valueChanged(m_label->text());
}

void QtInputEditWidget::setValue(const QString &value)
{
    QString oldValue = m_label->text();
    if (oldValue != value) {
        m_label->setText(value);
...
bool QtInputEditWidget::eventFilter(QObject *obj, QEvent *ev)
{
    if (obj == m_button) {
        switch (ev->type()) {
        case QEvent::KeyPress:
...
class QtInputEditorFactoryPrivate : public EditorFactoryPrivate<QtInputEditWidget>
{
    QtInputEditorFactory *q_ptr;
    Q_DECLARE_PUBLIC(QtInputEditorFactory)
public:
...
void QtInputEditorFactoryPrivate::slotPropertyChanged(QtProperty *property,
                                                      const QString &value)
{
    const PropertyToEditorListMap::iterator it = m_createdEditors.find(property);
    if (it == m_createdEditors.end())
        return;
    QListIterator<QtInputEditWidget *> itEditor(it.value());

    while (itEditor.hasNext())
        itEditor.next()->setValue(value);
}

void QtInputEditorFactoryPrivate::slotSetValue(const QString &value)
{
    QObject *object = q_ptr->sender();
    const EditorToPropertyMap::ConstIterator ecend = m_editorToProperty.constEnd();
    for (EditorToPropertyMap::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
...
QtInputEditorFactory::QtInputEditorFactory(QObject *parent) :
QtAbstractEditorFactory<QtStringPropertyManager>(parent),
d_ptr(new QtInputEditorFactoryPrivate())
{
    d_ptr->q_ptr = this;
}

QtInputEditorFactory::~QtInputEditorFactory()
{
    qDeleteAll(d_ptr->m_editorToProperty.keys());
    delete d_ptr;
}

void QtInputEditorFactory::connectPropertyManager(QtStringPropertyManager *manager)
{
    connect(manager, SIGNAL(valueChanged(QtProperty*,QString )),
            this, SLOT(slotPropertyChanged(QtProperty*,QString )));
}

QWidget *QtInputEditorFactory::createEditor(QtStringPropertyManager *manager,
        QtProperty *property, QWidget *parent)
{
    QtInputEditWidget *editor = d_ptr->createEditor(property, parent);
    editor->setValue(manager->value(property));
    connect(editor, SIGNAL(valueChanged(QString)), this, SLOT(slotSetValue(QString)));
    connect(editor, SIGNAL(destroyed(QObject *)), this, SLOT(slotEditorDestroyed(QObject *)));
    return editor;
...
void QtInputEditorFactory::disconnectPropertyManager(QtStringPropertyManager *manager)
{
    disconnect(manager, SIGNAL(valueChanged(QtProperty*,QString)), this, SLOT(slotPropertyChanged(QtProperty*,QString)));
}

// QtButtonEditWidget
class QtButtonEditWidget  : public QWidget
{
	Q_OBJECT
public:
	QtButtonEditWidget(QWidget *parent);
	bool eventFilter(QObject *obj, QEvent *ev);
...
QtButtonEditWidget::QtButtonEditWidget(QWidget *parent) 
: QWidget(parent)
, m_label(NULL)
, m_button(NULL)
{
	...
	m_button->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Ignored);
	m_button->setFixedWidth(70);
	setFocusProxy(m_button);
	setFocusPolicy(m_button->focusPolicy());

	QString btnName = DialogMgr::GetButtonName();
	m_button->setText(btnName);

	m_button->installEventFilter(this);
	connect(m_button, SIGNAL(clicked()), this, SLOT(buttonClicked()));
	lt->addWidget(m_button);

	connect(m_label, SIGNAL(editingFinished()), this, SLOT(EditingFinished()));
}

void QtButtonEditWidget::buttonClicked()
{
	QString newValue;
	QString oldValue = m_label->text();
	QString dialogName = DialogMgr::GetDialogName();
	int ok = DialogMgr::openDialog(this, dialogName, 0);
...
void QtButtonEditWidget::EditingFinished()
{
	emit valueChanged(m_label->text());
}

void QtButtonEditWidget::setValue(const QString &value)
{
	QString oldValue = m_label->text();
	if (oldValue != value) {
		m_label->setText(value);
...
bool QtButtonEditWidget::eventFilter(QObject *obj, QEvent *ev)
{
	if (obj == m_button) {
		switch (ev->type()) {
			case QEvent::KeyPress:
...
class QtButtonEditorFactoryPrivate : public EditorFactoryPrivate<QtButtonEditWidget>
{
	QtButtonEditorFactory *q_ptr;
	Q_DECLARE_PUBLIC(QtButtonEditorFactory)
public:
...
void QtButtonEditorFactoryPrivate::slotPropertyChanged(QtProperty *property,
													  const QString &value)
{
	const PropertyToEditorListMap::iterator it = m_createdEditors.find(property);
	if (it == m_createdEditors.end())
		return;
	QListIterator<QtButtonEditWidget *> itEditor(it.value());

	while (itEditor.hasNext())
		itEditor.next()->setValue(value);
}

void QtButtonEditorFactoryPrivate::slotSetValue(const QString &value)
{
	QObject *object = q_ptr->sender();
	const EditorToPropertyMap::ConstIterator ecend = m_editorToProperty.constEnd();
	for (EditorToPropertyMap::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
...
QtButtonEditorFactory::QtButtonEditorFactory(QObject *parent) :
QtAbstractEditorFactory<QtStringPropertyManager>(parent),
d_ptr(new QtButtonEditorFactoryPrivate())
{
	d_ptr->q_ptr = this;
}

QtButtonEditorFactory::~QtButtonEditorFactory()
{
	qDeleteAll(d_ptr->m_editorToProperty.keys());
	delete d_ptr;
}

void QtButtonEditorFactory::connectPropertyManager(QtStringPropertyManager *manager)
{
	connect(manager, SIGNAL(valueChanged(QtProperty*,QString )),
		this, SLOT(slotPropertyChanged(QtProperty*,QString )));
}

QWidget *QtButtonEditorFactory::createEditor(QtStringPropertyManager *manager,
											QtProperty *property, QWidget *parent)
{
	QtButtonEditWidget *editor = d_ptr->createEditor(property, parent);
	editor->setValue(manager->value(property));
	connect(editor, SIGNAL(valueChanged(QString)), this, SLOT(slotSetValue(QString)));
	connect(editor, SIGNAL(destroyed(QObject *)), this, SLOT(slotEditorDestroyed(QObject *)));
	return editor;
...
void QtButtonEditorFactory::disconnectPropertyManager(QtStringPropertyManager *manager)
{
	disconnect(manager, SIGNAL(valueChanged(QtProperty*,QString&)), this, SLOT(slotPropertyChanged(QtProperty*,QString&)));
}
