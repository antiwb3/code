...
class QtStringMgrPropertyType
{
};

class QtInputEditPropertyType
{
};

class QtButtonEditPropertyType
{
};
...
Q_DECLARE_METATYPE(QtStringMgrPropertyType)
Q_DECLARE_METATYPE(QtInputEditPropertyType)
Q_DECLARE_METATYPE(QtButtonEditPropertyType)
...
int QtVariantPropertyManager::stringMgrTypeId()
{
    return qMetaTypeId<QtStringMgrPropertyType>();
}

int QtVariantPropertyManager::inputEditTypeId()
{
	return qMetaTypeId<QtInputEditPropertyType>();
}

int QtVariantPropertyManager::buttonEditTypeId()
{
	return qMetaTypeId<QtButtonEditPropertyType>();
}
    ...
    : QtAbstractPropertyManager(parent)
{
    ...
    int stringMgrId = stringMgrTypeId();
    QtStringPropertyManager *stringMgrPropertyManager = new QtStringPropertyManager(this);
    d_ptr->m_typeToPropertyManager[stringMgrId] = stringPropertyManager;
    d_ptr->m_typeToValueType[stringMgrId] = QVariant::String;
    d_ptr->m_typeToAttributeToAttributeType[QVariant::String][d_ptr->m_regExpAttribute] =
        QVariant::RegExp;

    connect(stringMgrPropertyManager, SIGNAL(valueChanged(QtProperty *, const QString &)),
        this, SLOT(slotValueChanged(QtProperty *, const QString &)));
    connect(stringMgrPropertyManager, SIGNAL(regExpChanged(QtProperty *, const QRegExp &)),
        this, SLOT(slotRegExpChanged(QtProperty *, const QRegExp &)));

	int inputEditId = inputEditTypeId();
	QtStringPropertyManager *inputEditPropertyManager = new QtStringPropertyManager(this);
	d_ptr->m_typeToPropertyManager[inputEditId] = inputEditPropertyManager;
	d_ptr->m_typeToValueType[inputEditId] = QVariant::String;
	d_ptr->m_typeToAttributeToAttributeType[QVariant::String][d_ptr->m_regExpAttribute] =
		QVariant::RegExp;

	connect(inputEditPropertyManager, SIGNAL(valueChanged(QtProperty *, const QString &)),
		this, SLOT(slotValueChanged(QtProperty *, const QString &)));
	connect(inputEditPropertyManager, SIGNAL(regExpChanged(QtProperty *, const QRegExp &)),
		this, SLOT(slotRegExpChanged(QtProperty *, const QRegExp &)));

	int buttonEditId = buttonEditTypeId();
	QtStringPropertyManager *buttonEditPropertyManager = new QtStringPropertyManager(this);
	d_ptr->m_typeToPropertyManager[buttonEditId] = buttonEditPropertyManager;
	d_ptr->m_typeToValueType[buttonEditId] = QVariant::String;
	d_ptr->m_typeToAttributeToAttributeType[QVariant::String][d_ptr->m_regExpAttribute] =
		QVariant::RegExp;

	connect(buttonEditPropertyManager, SIGNAL(valueChanged(QtProperty *, const QString &)),
		this, SLOT(slotValueChanged(QtProperty *, const QString &)));
	connect(buttonEditPropertyManager, SIGNAL(regExpChanged(QtProperty *, const QRegExp &)),
		this, SLOT(slotRegExpChanged(QtProperty *, const QRegExp &)));
}
...
public:
    ...
    QtStringMgrEditorFactory   *m_stringEditorFactory;
	QtInputEditorFactory	   *m_InputEditorFactory;
	QtButtonEditorFactory	   *m_buttonEditorFactory;
    ...
    : QtAbstractEditorFactory<QtVariantPropertyManager>(parent)
{
    ...
    d_ptr->m_stringEditorFactory = new QtStringMgrEditorFactory(this);
    const int stringMgrId = QtVariantPropertyManager::stringMgrTypeId();
    d_ptr->m_factoryToType[d_ptr->m_stringEditorFactory] = stringMgrId;
    d_ptr->m_typeToFactory[stringMgrId] = d_ptr->m_stringEditorFactory;

	d_ptr->m_InputEditorFactory = new QtInputEditorFactory(this);
	const int inputEditId = QtVariantPropertyManager::inputEditTypeId();
	d_ptr->m_factoryToType[d_ptr->m_InputEditorFactory] = inputEditId;
	d_ptr->m_typeToFactory[inputEditId] = d_ptr->m_InputEditorFactory;

	d_ptr->m_buttonEditorFactory = new QtButtonEditorFactory(this);
	const int buttonEditId = QtVariantPropertyManager::buttonEditTypeId();
	d_ptr->m_factoryToType[d_ptr->m_buttonEditorFactory] = buttonEditId;
	d_ptr->m_typeToFactory[buttonEditId] = d_ptr->m_buttonEditorFactory;
}
...
void QtVariantEditorFactory::connectPropertyManager(QtVariantPropertyManager *manager)
{
    ...
    while (itString.hasNext())
    {
        ...
        d_ptr->m_stringEditorFactory->addPropertyManager(manager);
		d_ptr->m_InputEditorFactory->addPropertyManager(manager);
		d_ptr->m_buttonEditorFactory->addPropertyManager(manager);
    }
...
void QtVariantEditorFactory::disconnectPropertyManager(QtVariantPropertyManager *manager)
{
    ...
    while (itString.hasNext())
    {
        ...
        d_ptr->m_stringEditorFactory->removePropertyManager(manager);
		d_ptr->m_InputEditorFactory->removePropertyManager(manager);
		d_ptr->m_buttonEditorFactory->removePropertyManager(manager);
    }
    QList<QtDatePropertyManager *> datePropertyManagers = qFindChildren<QtDatePropertyManager *>(manager);
    QListIterator<QtDatePropertyManager *> itDate(datePropertyManagers);
    while (itDate.hasNext())
