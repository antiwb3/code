...
public:
   ...
   static HMODULE WINAPI GetModuleHandleRaw(PCSTR pszProcName);
private:
...
private:
   ...
   static void WINAPI ReplaceIATInAllMods(PCSTR pszCalleeModName, 
      PROC pfnOrig, PROC pfnHook);

   // Replaces a symbol's address in all modules' import sections
   static void WINAPI ReplaceIATInOneMod(PCSTR pszCalleeModName, 
      PROC pfnOrig, PROC pfnHook, HMODULE hmodCaller);

   // Replaces a symbol's address in a module's export sections
   static void ReplaceEATInOneMod(HMODULE hmod, PCSTR pszFunctionName, PROC pfnNew);
...
private:
   ...
   static CAPIHook sm_WritePrivateProfileString;
   
   //static CAPIHook sm_GetModuleHandleA;
};
